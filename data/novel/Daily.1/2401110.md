...
目前 [scroll-driven animations](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_scroll-driven_animations) 的浏览器兼容性不是很好，故编写一个 demo 暂存在这个页面。利用 JS 实现的滚动条要比原生消耗更多性能，在反复调试了一下午后还是选择再观望一阵子。
...
- 当 ``w < d`` 时，若将滚动条区域看作文档，滑块看作窗口，则可以得到比例关系 ``t / w = w / d``，
...
从上文得知，滑块的最大移动距离相当于 ``w - t``，同理，文档的最大移动距离相当于 ``d - w``；则当鼠标拖动滑块移动 ``x`` 个像素时，窗口要移动的距离为 ``x * (d - w) / (w - t)``：
...
useEventListener("mousemove", useThrottleFn((event) => {
    if (isPressed) {
        const duration = event.y - startY;
        window.scrollBy({
            top: duration * (docHeight.value - winHeight.value) / (winHeight.value - thumbHeight.value)
        });
        startY = event.y;
    }
}, 16.6));

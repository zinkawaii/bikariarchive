...
> [MDN](https://developer.mozilla.org/zh-CN/docs/Web/API/BroadcastChannel)：BroadcastChannel 接口代理了一个命名频道，可以让指定 origin 下的任意 browsing context 来订阅它。它允许同源的不同浏览器窗口，Tab 页，frame 或者 iframe 下的不同文档之间相互通信。通过触发一个 message 事件，消息可以广播到所有监听了该频道的 BroadcastChannel 对象。
...
然后自定义一个事件，并以``storage:${key}``的格式命名。这种做法的好处是，不用在单个监听事件中使用``if / else``或``switch``语句进行冗长的判断，而是将其分割成多个监听事件。
